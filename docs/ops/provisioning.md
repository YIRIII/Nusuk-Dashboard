...
This runbook covers the one-time host setup for Hadaq Tracker v2. Steps marked **[manual]** require you to click through a console; all others are scripted.
...
export HADAQ_HOST=<public-ip>
scp scripts/ops/bootstrap.sh ubuntu@$HADAQ_HOST:/tmp/
ssh ubuntu@$HADAQ_HOST "sudo bash /tmp/bootstrap.sh"
```
...
- Creates a `hadaq` service user, Docker group membership
- Installs the systemd units for `hadaq-api` and `cloudflared`
...
5. Add a public hostname under the tunnel (e.g., `tracker.hadaq-platform.com`) → Service: `HTTP://localhost:3001`
...
docker buildx build --platform linux/arm64 --load -t hadaq-api:latest -f docker/Dockerfile .
docker save hadaq-api:latest | ssh ubuntu@$HADAQ_HOST "sudo docker load"
ssh ubuntu@$HADAQ_HOST "sudo systemctl restart hadaq-api"
```
...
curl https://tracker.hadaq-platform.com/health
# { "ok": true, "ts": "..." }
```
